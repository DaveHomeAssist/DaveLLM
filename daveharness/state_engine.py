...
from .engine import (
    ...
    _canonical_json, _invoke_model, _transcript_revision, run_tool,
)
from .parser import parse_tool_calls
from .policy import RunPolicyContext, ToolPolicy
from .registry import ToolRegistry
from .schema import SchemaValidationError
from .state import ApprovalDecision, PendingToolCall, RunSnapshot, transition_run
...
def _terminal(
    ...
    changes: dict[str, Any] = {"model_inflight": False}
    if snapshot.pending_call is not None:
        changes["pending_call"] = None
    next_snapshot = transition_run(snapshot, status, updated_at=_now(clock).isoformat(), **changes)
...
def _progress(
    ...
    next_snapshot = transition_run(snapshot, "running", updated_at=_now(clock).isoformat(), **changes)
        ...
        if decision.action == "pause" and definition is not None:
            try:
                ...
                next_snapshot = transition_run(
                    snapshot, "approval_required", updated_at=created.isoformat(),
...
async def resume_run(
    ...
    *, policy: ToolPolicy | None = None, clock: Clock = _utc_now,
    ...
    if snapshot.status == "created":
        started = transition_run(snapshot, "running", updated_at=_now(clock).isoformat())
    ...
    while True:
        ...
        if snapshot.queued_calls:
            drained = await _drain_calls(store, snapshot, registry, current_policy, clock)
        ...
        try:
            message = await _invoke_model(
                invoke_model, snapshot.transcript, registry.model_schemas(),
        ...
        if not calls:
            transcript.append(assistant)
                ...
                snapshot, "completed", updated_at=_now(clock).isoformat(),
...
async def decide_run(
    ...
    if decision.decision == "reject":
        rejected = transition_run(
            snapshot, "approval_rejected", updated_at=now.isoformat(),
            pending_call=None,
    ...
    reserved = transition_run(
        snapshot, "running", updated_at=now.isoformat(), pending_call=None,
