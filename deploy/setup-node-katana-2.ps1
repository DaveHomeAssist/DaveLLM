...
Write-Host "=== DaveLLM Node Setup: Katana 2 (RTX 4060) — Vision ===" -ForegroundColor Cyan
...
Write-Host "Pulling llava:13b (vision/multimodal)..." -ForegroundColor Yellow
ollama pull llava:13b

Write-Host "Pulling llava:7b (fast vision fallback)..." -ForegroundColor Yellow
...
Write-Host "  {`"id`": `"node-katana-2`", `"name`": `"Katana 2 — LLaVA 13B`", `"url`": `"http://${ip}:11434`"}"
