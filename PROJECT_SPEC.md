...
|---|---|
| Product | DaveLLM |
| Status | Implemented local application with in-process DaveHarness `0.8.0` boundary |
...
The in-repository `daveharness` package is versioned independently at `0.8.0`. It is an internal importable package, not a published distribution, service, repository, CLI, or remote protocol. Future package changes follow independent SemVer while DaveLLM retains its own root `VERSION`.
...
- The DaveHarness library boundary is implemented in-process at `0.8.0`, with the legacy root import preserved as a compatibility shim.
...
- DaveHarness `0.6.0` adds opt-in cooperative cancellation, injected stoppable runners, and absolute run/model/tool deadlines. DaveHarness `0.7.0` adds metadata-only ordered lifecycle events. DaveHarness `0.8.0` adds a bounded run store and instance-owned facade. Legacy synchronous handlers retain response-deadline behavior.
