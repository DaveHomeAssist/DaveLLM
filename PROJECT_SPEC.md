...
| `DAVE_ENABLE_EXTENDED_TOOLS` | No | Opt-in for the read-only `file.list`, `file.search`, `file.read_lines`, `md.outline`, `md.section`, `git.status`, `git.diff`, `git.log`, and `git.show`, honored only with `DAVE_ENABLE_TOOLS`; false. See `docs/DAVELLM_TOOLS.md`. |
...
- DaveHarness `0.5.0` adds serializable run snapshots, exact approval decisions, and compare-and-swap resume operations. DaveLLM's existing routes continue using the legacy adapter; H7 adds separate lifecycle routes.
- DaveHarness `0.6.0` adds opt-in cooperative cancellation, injected stoppable runners, and absolute run/model/tool deadlines. DaveHarness `0.7.0` adds metadata-only ordered lifecycle events. DaveHarness `0.8.0` adds a bounded run store and instance-owned facade. DaveHarness `0.9.0` adds DaveLLM lifecycle integration and a frozen BRAIN context snapshot. Legacy synchronous handlers retain response-deadline behavior.
