...
The generic implementation is split across `daveharness/contracts.py`, `registry.py`, `schema.py`, `parser.py`, and `engine.py`, and exposed by `daveharness/__init__.py`. `daveharness/executor.py` and root `tool_executor.py` remain compatibility re-exports. DaveLLM imports the package API directly, while concrete tools and the model adapter remain in `app.py`.
...
DaveHarness `0.3.0` adds an internal version-one envelope for existing parsed calls, pending calls, tool executions, and outcomes. `encode_contract` and `decode_contract` validate complete payloads and own nested JSON copies. DaveHarness `0.4.0` adds injected policy decisions, definition fingerprints, and immutable budgets. The legacy `ExecutorOutcome.to_dict()` and DaveLLM HTTP responses are unchanged. DaveHarness `0.5.0` adds versioned run snapshots, exact approval decisions, and compare-and-swap resume operations. DaveHarness `0.6.0` adds opt-in cooperative cancellation and absolute deadlines without changing the legacy route. DaveHarness `0.7.0` adds metadata-only ordered events. DaveHarness `0.8.0` adds a bounded run store and instance-owned facade.
