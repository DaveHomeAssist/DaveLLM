...
Reservations are persisted before dispatch in a shared authorization ledger under the disposable root. Subsequent diagnostic/recovery attempts inherit the original start time and charged tokens. HTTP failures stop the batch and record sanitized server diagnostics. Interrupted or unknown-usage requests retain their entire reservation; restarting the script never resets the approved resource allowance.
