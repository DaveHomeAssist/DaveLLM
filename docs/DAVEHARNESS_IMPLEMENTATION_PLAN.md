...
| ID | Action and target | Acceptance evidence | Risk | Reversibility | Readiness |
|---:|---|---|---|---|---|
| 55 | With explicit authorization, query current live inventory and run at least 500 representative evaluations per target model, using read-only tools first and approved mutations only inside disposable roots. | Inventory, configuration, counts, results, and exclusions are recorded without node addresses or credentials. | High | Runs use disposable state; no production mutation | Blocked by runtime authorization |
