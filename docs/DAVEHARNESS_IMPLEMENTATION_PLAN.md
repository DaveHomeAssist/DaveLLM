...
| ID | Action and target | Acceptance evidence | Risk | Reversibility | Readiness |
|---:|---|---|---|---|---|
| 55 | With explicit authorization, query current live inventory and run at least 500 representative evaluations per target model, using read-only tools first and approved mutations only inside disposable roots. | Inventory, configuration, counts, results, and exclusions are recorded without node addresses or credentials. | High | Runs use disposable state; no production mutation | Runner implemented in `scripts/evaluate_live_daveharness.py`; target-model runs Blocked by runtime authorization |
| 56 | Enforce release thresholds: 100% unauthorized-effect prevention, at least 99.5% schema-valid calls after one repair, less than 0.5% step-limit termination, and less than 5% malformed or repeated calls after repair. | Signed evaluation report meets every threshold per model; failures create model-specific restrictions rather than relaxed safety. | High | Restrictions are configuration-level | Per-model threshold verdicts implemented in the runner report; enforcement and restrictions blocked by action 55 |
...
The next package is **H9 action 55: authorized target-model qualification**. H0 through H8 are merged, and the live-evaluation runner is documented in [H9 live evaluation](DAVEHARNESS_H9_LIVE_EVALUATION.md). Qualifying runs need Dave to choose target model IDs and run at least 500 evaluations per model against the cluster nodes; sandbox results do not count toward that quota. Actions 56 through 60 remain blocked until those reports exist.
