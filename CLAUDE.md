...
- `DAVE_ENABLE_EXTENDED_TOOLS` is a separate opt-in for extended tools and is honored only when `DAVE_ENABLE_TOOLS` is also on. It registers no tools yet, so the default catalog and its definition fingerprints are unchanged. Shared helpers for those tools live in `davellm_files.py`; keep new `app.py` code below the tool handlers, because handler line numbers are part of the fingerprints (`tests/test_tool_catalog_provenance.py` pins them).
- Extended tools admit every path through `resolve_extended_tool_path`, which wraps `resolve_tool_path` with the secret denylist: `.env`, `.env.*`, `*.pem`, `*.key`, `id_rsa`, `id_rsa*`, `*.p12`, `.ssh`, `.aws`, and `.gnupg`, matched case-insensitively against every path component before and after symlink resolution. Every refusal, including a root escape, raises the same `PathNotAllowed` message. `walk_tree` never lists, enters, or counts secrets and omits symlinks that leave the root. The qualified `file.read` does not apply the denylist; extending it is a separate decision. New file tools must pass the shared contract in `tests/tool_contract.py` against the `hostile_tree` fixture.
